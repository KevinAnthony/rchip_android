...
public class VideoRemote extends Activity implements OnClickListener {
	...
	Log.d(Consts.LOG_TAG, "onCreate: MS_Watch_Movie");
	...
	if (btn.getId() == R.id.MSWMfullscreen) {
		...
		if (firstPlay) {
		    String rootPath;
		    try {
			rootPath = Soap.getRootValue(RemoteMain.msb_desthost);
			Log.d(Consts.LOG_TAG, "hostname:" + RemoteMain.msb_desthost);
	...
	protected Boolean doInBackground(String... incoming) {
		...
		Log.e(Consts.LOG_TAG, "Error sendcmd " + cmd + ":" + e.getMessage());
