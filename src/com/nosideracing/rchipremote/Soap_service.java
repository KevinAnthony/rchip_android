...
import com.nosideracing.rchipremote.backendservice;
...
public class Soap_service extends Service {
	...
	public int onStartCommand(Intent intent, int startFlags, int startId) {
		 ...
		 */
		try {
			mOpenHelper = new Database(getApplicationContext());
			...
			boolean ktornot = incoming
					.getBoolean("SETTING_KTORRENTNOTIFICATION");
			set_ktorrent_notifications(ktornot);
		} catch (Exception e) {
			Log.e(Consts.LOG_TAG, "Could not read file 1 "
					+ e.getMessage());
		...
		Log.i(Consts.LOG_TAG, "Soap Messaging Service Started");
	...
	public void onRebind(Intent intent) {
		...
		if (incoming != null) {
			try {
				...
				HOSTNAME = incoming.get("SETTING_SOURCENAME").toString();
				DESTHOSTNAME = incoming.get("SETTING_DESTNAME").toString();
				INT_NETWORK_NAME = incoming
						.get("SETTING_INTERNAL_NETWORK_NAME").toString();
				EXT_DELAY = incoming.getInt("SETTING_EXTERNAL_DELAY");
				INT_DELAY = incoming.getInt("SETTING_INTERNAL_DELAY");
				boolean ktornot = incoming
						.getBoolean("SETTING_KTORRENTNOTIFICATION");
				...
				Log.w(Consts.LOG_TAG, "onRebind Error");
		...
		Log.d(Consts.LOG_TAG, "onRebind: msserivce");

	}

	@Override
	public IBinder onBind(Intent intent) {
		Log.d(Consts.LOG_TAG, "OnBind: msserivce");
		return interfaceBinder;
	}

	@Override
	public boolean onUnbind(Intent intent) {
		Log.d(Consts.LOG_TAG, "onUnbind: msservice");
		/*
	...
	public void onCreate() {
		super.onCreate();
		Log.d(Consts.LOG_TAG, "onCreate: msservice");
	}

	@Override
	public void onDestroy() {
		super.onDestroy();
		Log.d(Consts.LOG_TAG, "onDestroy: msservice");
	}
	...
	private boolean registerForMessages(String active) {
		 ...
		 */
		SoapObject request = new SoapObject(Consts.NAMESPACE,
				Consts.METHOD_NAME_REGISTERMESSAGES);
	...
	private boolean registerDevice(boolean state) {
		...
		state = true;
		SoapObject request = new SoapObject(Consts.NAMESPACE,
				Consts.METHOD_NAME_REGISTERACTIVEDEVICE);
		request.addProperty("host", HOSTNAME);
	...
	private boolean sendCmdToSoap(String cmd, String cmdTxt) {
		...
		if (cmd == "STRB") {
			Log.e(Consts.LOG_TAG, "STRB DEPRECIATED");
			return registerDevice(true);
		} else if (cmd == "SPRB") {
			Log.e(Consts.LOG_TAG, "SPRB DEPRECIATED");
	...
	private String[] setStatusNotification(String tickerString,
		...
		try {
			...
			PendingIntent contentIntent = PendingIntent.getActivity(context, 0,
					new Intent(this, VideoList.class), 0);
			...
			mNotificationManager.notify(Consts.NOTIFICATION_ID,
					notification);
			...
			Log.e(Consts.LOG_TAG, "SetStatusNotification:"
					+ e.getMessage());
			Log.e(Consts.LOG_TAG, "", e);
			return null;
		}
	...
	class RunTask implements Runnable {
		...
		public void run() {
			...
			if (running) {
				...
				Log.i(Consts.LOG_TAG, "Exiting runnable");
			}
		}

		@SuppressWarnings("unchecked")
		private boolean checkMessages() {
			Log.i(Consts.LOG_TAG, "checking Messages within runnable");
			/*
	...
	class passDataToShowWindow extends
		...
		protected Boolean doInBackground(List<String[]>... params) {
			...
			try {
				...
				Log.e(Consts.LOG_TAG, "Could not insert "
						+ e.getMessage());
				Log.e(Consts.LOG_TAG, "", e);
			}
			return true;
