...
public class MS_show_list extends ExpandableListActivity {
	...
	if (type == ExpandableListView.PACKED_POSITION_TYPE_CHILD) {
	    ...
	    id = ((MyExpandableListAdapter) mAdapter).getlongID(groupPos, childPos);
	} else if (type == ExpandableListView.PACKED_POSITION_TYPE_GROUP) {
	    Toast.makeText(this, "Please Select a Valid Show", Toast.LENGTH_SHORT).show();
	    return true;
	}
	...
	i.putExtra("showString", torName.get((int) id) + " - " + torNumber.get((int) id) + " - "
	...
	public MyExpandableListAdapter(Context context) {
		...
		while ((k < torName.size()) && (torName.get(k).equals(groups.get(i)))) {
	...
	public View getChildView(int groupPosition, int childPosition, boolean isLastChild,
	    ...
	    holder.text.setText(children.get(groupPosition).get(childPosition).get(0));
	    holder.text2.setText(children.get(groupPosition).get(childPosition).get(1));
	    holder.text3.setText(children.get(groupPosition).get(childPosition).get(2));
    ...
    /*
     * class BackendServiceConnection implements ServiceConnection { public void
     * onServiceConnected(ComponentName className, IBinder boundService) {
     * backendService = backendservice.Stub .asInterface((IBinder)
     * boundService); Log.d(MS_constants.LOG_TAG,
     * "onServiceConnected: msremote"); }
     * 
     * public void onServiceDisconnected(ComponentName className) {
     * backendService = null; Log.d(MS_constants.LOG_TAG,
     * "onServiceDisconnected: msremote"); } }
     */
}
